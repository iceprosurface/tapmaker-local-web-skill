...
5. 根据项目或用户要求选择方向：横屏使用默认的 `--orientation landscape`，竖屏使用 `--orientation portrait`；无法从任务内项目代码、配置或截图判断时才沿用横屏默认值。用户要求独立组件树时增加 `--inspect`；要求左右编辑器工作台时使用 `--workbench`。用可持续会话启动 `uv run --project <skill-dir>/scripts tapmaker-local-web web --code <code-dir> --entry <entry> --orientation <orientation> --no-open`。保留服务器输出中的实际 URL；不要猜测端口或吞掉启动错误。
...
- `--inspect` 会编译期注入只存在于内存的 Lua 调试桥，并把精简组件树上报给 `/__tapmaker/component-tree`。`--workbench` 使用同一调试桥，将 Maker Runtime 作为带版本通讯的 iframe session 挂载到左右工作台。没有读到树时接口返回 warning 与可交给 Agent 的适配提示词。不得把调试桥写回项目或带入正式构建。
